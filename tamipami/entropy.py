...
def entropy_of_frequency_array(freq_array):
    ...
    entropy_per_column = -np.nansum(np.where(prob_array > 0, prob_array * np.log2(prob_array), 0), axis=0)
...
def find_max_gradient_and_zscore(cumm_ent_array: np.ndarray, zscore: list, orientation: str) -> pd.DataFrame:
    """
    ...
    for i in range(1,cumm_ent_array.shape[1]):
        t_ent = tot_ent(cumm_ent_array= cumm_ent_array, length=i + 1 , orientation=orientation)
        df = pd.DataFrame({"zscore": zscore, "t_ent": t_ent})
        df['gradient'] = np.gradient(df["zscore"], df['t_ent'],)
...
def analyze_clusters(df: pd.DataFrame) -> tuple[int,float]:
    ...
    return (int(target_row['pam_length'] + 1 ), float(target_row['zscore_at_max_gradient']))

def main(kmers: list[str] | np.ndarray | pd.Series, zscore: list[str] | np.ndarray | pd.Series, orientation: str) -> tuple[int, float]:
    """
    ...
    mgz_df = find_max_gradient_and_zscore(cumm_ent_array=cumm_ent_array, zscore=zscore, orientation=orientation)
