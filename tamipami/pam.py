...
class pamSeqExp:
    ...
    def __init__(
        ...
        Returns:
            None
        """
        assert all([ctl, exp, position]) ^ (
            multikmerdict is not None
        ), "please provide either clt, exp and position or a multikerlevel object"
    ...
    def _kmer_len(self, kmers: dict[str, int]) -> int:
        ...
        Raises:
            AssertionError if the length of all kmers is not the same.
        """
        kmerlen: int = None
    ...
    def _combine_single_pair(
        self, exper: dict[str:list], ctl: dict[str, list]
    ) -> pd.DataFrame:
    ...
    def _group_and_sum_kmers(
        ...
        """This function takes a df with longer kmers and sums up the counts at each shorter kmer. This is done so that
    ...
    def find_breakpoint(self, length, type="zscore"):
        assert type in [
            "zscore",
            "diff",
        ], "Parameter 'type' should be 'diff' or 'zscore'"
