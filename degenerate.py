...
def gen_all_combo(minimal_representations: list[set|list] ) ->set[str]:
    ...
    for combination in product(*[list(chars) if isinstance(chars, set) else [chars] for chars in minimal_representations]):
...
def create_degenerate(subset: set[str]) -> str|None:
    ...
    if subset == gen_all_combo(position_to_chars):
        return degererate_represenation(position_to_chars)
    else:
        return None
    ...
    def add_children(parent_node, parent_id, labels):
        if parent_node.left is not None:
            ...
            if left_node.left is None and left_node.right is None:
                ...
                tree.create_node(tag=labels[left_node.id], identifier=str(left_node.id), parent=parent_id)
            else:
        ...
        if parent_node.right is not None:
            ...
            if right_node.left is None and right_node.right is None:
                # Leaf node, add label using its index in the nodes list
                tree.create_node(tag=labels[right_node.id], identifier=str(right_node.id), parent=parent_id)
            else:
                tree.create_node(tag="", identifier=str(right_node.id), parent=parent_id)
